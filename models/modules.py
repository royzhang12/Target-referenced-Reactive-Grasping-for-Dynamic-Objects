...
class ApproachNet(nn.Module):
    ...
    def forward(self, seed_xyz, seed_features, end_points):
        ...
        top_view_inds_ = top_view_inds.contiguous().view(B, num_seed, 1, 1).expand(-1, -1, -1, 3).contiguous()
        template_views = generate_grasp_views(self.num_view).to(features.device) # (num_view, 3)
        template_views = template_views.contiguous().view(1, 1, self.num_view, 3).expand(B, num_seed, -1, -1).contiguous() #(B, num_seed, num_view, 3)
        vp_xyz = torch.gather(template_views, 2, top_view_inds_).squeeze(2) #(B, num_seed, 3)
        vp_xyz_ = vp_xyz.contiguous().view(-1, 3)
        batch_angle = torch.zeros(vp_xyz_.size(0), dtype=vp_xyz.dtype, device=vp_xyz.device)
        vp_rot = batch_viewpoint_params_to_matrix(-vp_xyz_, batch_angle).contiguous().view(B, num_seed, 3, 3)
...
class CloudCrop(nn.Module):
    ...
    """
    def __init__(self, nsample, seed_feature_dim, cylinder_radius=0.05, hmin=-0.02, hmax_list=[0.01,0.02,0.03,0.04], use_xyz=True, rotate_xyz=True, use_feat=False):
        super().__init__()
        ...
        for hmax in hmax_list:
            self.groupers.append(CylinderQueryAndGroup(
                cylinder_radius, hmin, hmax, nsample, use_xyz=use_xyz, rotate_xyz=rotate_xyz
            ))
        self.mlps = pt_utils.SharedMLP(mlps, bn=True)

    def forward(self, seed_xyz, pointcloud, vp_rot, features=None):
        """ Forward pass.
            Input:
                seed_xyz: [torch.FloatTensor, (batch_size,num_seed,3)]
                    coordinates of seed points
        ...
        grouped_features = grouped_features.contiguous().view(B, -1, num_seed*num_depth, self.nsample) # (batch_size, feature_dim, num_seed*num_depth, nsample)

        if self.use_feat:
            grouped_colors = grouped_features[:, 3:]
        ...
        vp_seed_features = vp_features.contiguous().view(B, -1, num_seed, num_depth, self.nsample)

        vp_features = F.max_pool2d(
            vp_features, kernel_size=[1, vp_features.size(3)]
        ) # (batch_size, mlps[-1], num_seed*num_depth, 1)
        vp_features = vp_features.contiguous().view(B, -1, num_seed, num_depth)

        if self.use_feat:
            vp_features_color = self.mlps2(
            grouped_colors
            ) # (batch_size, mlps[-1], num_seed*num_depth, nsample)

            vp_seed_features_color = vp_features_color.contiguous().view(B, -1, num_seed, num_depth, self.nsample)

            vp_features_color = F.avg_pool2d(
                vp_features_color, kernel_size=[1, vp_features_color.size(3)]
            ) # (batch_size, mlps[-1], num_seed*num_depth, 1)
            vp_features_color = vp_features_color.contiguous().view(B, -1, num_seed, num_depth)


        if features is None:
            return vp_features, vp_seed_features
        else:
            return vp_features, vp_seed_features, vp_features_color, vp_seed_features_color
